...
#region PRIVATE_VARIABLES
	...
	public void SetRadius(float scale)
	{
		...
		var d = m_ReferencePointParent.transform.position;
		m_ReferencePointParent.transform.position = m_ReferencePoints[0].transform.position;

		d -= m_ReferencePointParent.transform.position;

		foreach (var rp in m_ReferencePoints)
			rp.transform.localPosition += d;

		m_ReferencePointParent.transform.localScale = new Vector3(scale, scale, scale);

		foreach (var rp in m_ReferencePoints)
			rp.transform.parent = null;

		m_ReferencePointParent.transform.localScale = new Vector3(1,1,1);

		foreach (var rp in m_ReferencePoints)
			rp.transform.parent = m_ReferencePointParent.transform;

		p_radius = m_ReferencePoints[0].transform.localScale.x;
...
#region PUBLIC_CLASSES
	...
	/// </summary>
	public void Scale(float radius, bool scaleAttachedObjects = true)
	{

		Vector3 newSize = new Vector3(radius, radius, radius);
		var deltaScale = radius / p_radius;
		p_radius = radius;
		...
		m_Transform.localScale = newSize;
		m_CentralPoint.transform.parent.localScale = newSize;
		...
		foreach(ReferencePoint refPoint in m_ReferencePoints)
		{
			if(refPoint.GameObject)
			{
				refPoint.transform.position = refPointPositions[0] + ((refPointPositions[index] - refPointPositions[0]) * deltaScale);
		...
		//DG Check This
		if(!scaleAttachedObjects && radius > 0)
		{
			float inverseScale = 1.0f/radius;
